...
type attr struct {
	...
	// When we build the att/gatt table for a server, we make sure each attribute:
	rhandler ReadHandler
	whandler WriteHandler
	nhandler NotifyHandler // only valid for Characteristic's valued descriptor

	// Pointer back to the embeddeding Serverce/Characteristic/Descriptor
	pvt interface{}
}
...
func generateAttributes(name string, ss []*Service, base uint16) *attrRange {
	var aa []attr
	h := base
	last := len(ss) - 1
	for i, s := range ss {
...
func generateCharAttributes(c *Characteristic, h uint16) (uint16, []attr) {
	...
	d.h = h
	d.pvt = d
	binary.LittleEndian.PutUint16(c.value[1:], h)
	h++

	aa := []attr{c.attr, d.attr}
