...
func newCentral(d *Device, h *attrRange, l2conn io.ReadWriteCloser, m bool) *central {
	return &central{
		dev:         d,
		attrs:       h,
		mtu:         23,
...
func (c *central) MTU() int { return int(c.mtu) }
...
func (c *central) handleReadByType(b []byte) []byte {
	...
	uuidLen := -1
	for _, h := range c.attrs.Subrange(start, end) {
		if !h.typ.Equal(t) {
			continue
		}
		if (h.secure&CharRead) != 0 && c.security > securityLow {
...
func (c *central) handleReadBlob(b []byte) []byte {
	...
	w := newL2capWriter(c.mtu)
	w.WriteByteFit(attOpReadBlobReq)
	w.Chunk()
	w.WriteFit(v)
...
func (c *central) handleWrite(reqType byte, b []byte) []byte {
	...
	if !a.typ.Equal(AttrClientCharacteristicConfigUUID) {
		...
		d := a.pvt.(*Descriptor)
		r := Request{Central: c, Characteristic: d.char}
	...
	old := binary.LittleEndian.Uint16(a.value)
...
func (c *central) sendNotification(char *Characteristic, data []byte) (int, error) {
	w := newL2capWriter(c.mtu)
	w.WriteByteFit(attOpHandleNotify)
	w.WriteUint16Fit(char.valued.h)
	w.WriteFit(data)
...
func (c *central) startNotify(char *Characteristic, maxlen int) {
	c.notifiersmu.Lock()
	defer c.notifiersmu.Unlock()
	...
	c.notifiers[char] = n
	char.valued.nhandler.ServeNotify(Request{Central: c, Characteristic: char}, n)
}

func (c *central) stopNotify(char *Characteristic) {
	c.notifiersmu.Lock()
	defer c.notifiersmu.Unlock()
